...
class KubectlApiResourcesTest extends Specification {
	def "Kubectl api-resources discovery should work"() {
		given:
		...
		when:
		def allResources = Kubectl.apiResources()
				.apiClient(apiClient)
				.execute()
		then:
		allResources.any { it.group == "" && it.kind == "Pod" }
		allResources.any { it.group == "apps" && it.kind == "Deployment" }
		allResources.any { it.group == "networking.k8s.io" && it.kind == "Ingress" }
	}
}
